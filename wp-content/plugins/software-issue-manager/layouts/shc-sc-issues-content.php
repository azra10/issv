...
<tr>
    ...
    <td class="search-results-row"><?php echo do_shortcode("[wpas_woo_product_woo_issue con_name='woo_issue' app_name='software_issue_manager' type='list_ol' post= " . get_the_ID() . "]"); ?> </td>
    <?php
} ?> <?php if (shortcode_exists('wpas_edd_product_edd_issue')) { ?> 
    <td class="search-results-row"><?php echo do_shortcode("[wpas_edd_product_edd_issue con_name='edd_issue' app_name='software_issue_manager' type='list_ol' post= " . get_the_ID() . "]"); ?> </td>
    <?php
} ?> 
</tr>