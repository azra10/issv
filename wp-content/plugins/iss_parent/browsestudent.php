...
<div class="row">
    <nav aria-label="Page navigation">
            ...
            <?php
                ...
                if (! isset ( $_GET ['initial'] )) {  echo "  active\""; }
                echo "\"><a class=\"page-link\" href=\"admin.php?page=students_home\">All</a></li>";
                ?>  </li>&nbsp;&nbsp;
                <?php                                                           
                $letters = array ( 'KG', '1', '2', '3', '4', '5', '6', '7', '8', 'YG', 'YB', 'XX', 'New', 'Archived');
                $item = "";
 ...
 <div>            
    <form action="" method="post" class="navbar-form">
      <?php wp_nonce_field( 'iss_student_search','iss_student_search_nonce' ); ?>
        <!--select name="bulkaction">
              <option selected value="0">Bulk Action</option> <option>Archive</option> <option>Email</option>
        </select>
        <button id="actionbutton" name="actionbutton" class="button-primary" disabled=""> </i> Submit </button-->
        <input type="text" style="margin-left:300px;" name="keyword"
    ...
    <tbody>
        <?php foreach ($result_set as $row) { ?>
            ...
            <td>
                <?php if (iss_current_user_can_editparent() && ($row['StudentStatus'] == 'active')) { ?>
                <a
